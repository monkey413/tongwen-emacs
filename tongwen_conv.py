...
def main():
    ...
    tongwen = TongWenConv()
    data = infile.read()
    enc = chardet.detect(data[:10000]) 
    
    if enc['encoding'] == 'GBK' or enc['encoding'] == 'GB2312':
        enc['encoding'] = 'gb18030' 
       
    #print enc
    ret = tongwen.conv_zh(unicode(data, enc['encoding']), op)
    #ret = tongwen.conv_zh(unicode(data, 'utf8'), op)
    outfile.write(ret.encode('utf8',errors='ignore'))
